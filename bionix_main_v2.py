...
class EMGTransformerEncoder(nn.Module):
    ...
    def forward(self, emg_signal):
        ...
        emg_graph_attention, attn = self.emg_gat(emg_signal, edge_index, return_attention_weights=True)
        attn_to_viz = emg_graph_attention.detach().numpy()
        k = 5
        knn = NearestNeighbors(n_neighbors=k, metric='cosine') 
        knn.fit(attn_to_viz)
        
        distances, indices = knn.kneighbors(attn_to_viz)
        ...
        for i in range(5):
            for j in range(5):
                if j in indices[i]:
                    distance_matrix[i, j] = distances[i][list(indices[i]).index(j)]
                else:
                    distance_matrix[i, j] = np.nan

        plt.imshow(distance_matrix, cmap='viridis')
        plt.colorbar(label='Distance')
        plt.title('KNN Distance Matrix')
        plt.xlabel('Channel')
        plt.ylabel('Channel')
        plt.show()
        ...
        emg_graph_struct_embeds = self.spatial_emg_encoder(emg_signal, edge_index, edge_weight=weights)  # (5, 256)
...
class EEGGraphEncoder(nn.Module):
    ...
    def forward(self, eeg_graph):
        eeg_features = self.gat_encoder(eeg_graph.x, eeg_graph.edge_index, eeg_graph.edge_attr) # The "edge_attr" parameter is for "edge_weight" in GIN forward pass
        eeg_features = self.gin_transform(eeg_features, eeg_graph.edge_index, eeg_graph.edge_attr)  # Edge attr isnt necessary fro this attention layer
...
emg_data = np.load('bionix_kin_data_n10/emg_signals.npy')
